...
class PageBanner extends BlockBase {
  ...
  public function build() {
    ...
    $image_uri = FALSE;
    $show_video = FALSE;
    $desktop_url = $tablet_url = $mobile_url = FALSE;
    ...
    if ($node) {
      ...
      if ($node->hasField('field_banner_height')) {
        if ((!$node->get('field_banner_height')->isEmpty()) && ($node->get('field_banner_height')->first()->getValue()['value'] !== 'full')) {
          $size = $node->get('field_banner_height')->first()->getValue()['value'];
        }
      }
      ...
      if ($node->hasField('field_banner_video') && (!$node->get('field_banner_video')->isEmpty())) {
        $show_video = TRUE;
        $media = Media::load($node->get('field_banner_video')->first()->getValue()['target_id']);
        $file = File::load($media->getSource()->getSourceFieldValue($media));
        $video_url = $file->getFileUri();
        $video_url = \Drupal::service('file_url_generator')->generateAbsoluteString($video_url);

        $aria_label = (!$media->get('field_aria_label')->isEmpty()) ? ($media->get('field_aria_label')->first()->getValue()['value']) : NULL;
      }
    }
    ...
    if ($size != 'min') {
      if ($show_video) {
        return [
          '#theme' => 'oyster_page_banner_video',
          '#title' => $title,
